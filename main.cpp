...
void solve()
{
    ...
    while(hasToSolve)
    {
        ...
        if(reqId != -1)
        {
            int eId = requestsArr[reqId].endpointID;

            int minCache = 1000001;

            for(int i = 0; i < 1000; i++)
            {
                if(endpointsArr[eId].cacheLatency[i] < minCache &&
                   cachesArr[i].getCrtSize() + videoSizes[requestsArr[reqId].videoID])
                {
                    minCache = i;
                }
            }
            ...
            if(minCache != 1000001)
            {
                cachesArr[minCache].videos.push_back(requestsArr[reqId].videoID);
                swap(requestsArr[reqId], requestsArr[requestsArr.size() - 1]);
                requestsArr.pop_back();
            }
            else
                hasToSolve = false;
